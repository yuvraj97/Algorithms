...
h = HashTable()
for i in range(7):
    key, value=randrange(0,100),randrange(0,100)
    h[key] = value
for i in h:
    print(i)
'''
...
f=open('log.log','w')
f.close()
speed,insert,delete=0,0,0
for i in range(100):
    h,t = getRandomHashTable(1000000)
    insert += t/len(h)
    print(len(h))
    t=speedCheck(h)
    speed += t/len(h)
    deleted,t = removeHashTableRandomly(h)
    delete += t/deleted
    print(deleted,'items deleted')
    print(len(h))
f=open('log.log','a')
f.write("In 100 Trials On Average Inserting One key,val pair took"+ str(insert) +"sec\n")
f.write("And Searching One key,val pair took:"+str(speed)+" sec\n")
f.write("And deleting one key,val pair took:"+str(delete)+" sec")
f.close()
'''